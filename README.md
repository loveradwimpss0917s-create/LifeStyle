...
npm run check:images   # 画像規約チェック(26章c28・長辺1600px超/1MB超/ai-プレフィックスのalt欠落を警告)
npm run check:links    # リンク死活チェック(26章c14・デプロイ後の公開サイトを対象。ローカルではスキップ可)
```
...
4. `npm run check:images`(26章c28の警告チェック。長辺1600px超・1MB超の実写真、`ai-`プレフィックス画像のalt「イメージ写真」欠落を検出。ビルドは失敗させない)
5. `npx @lhci/cli autorun`(Lighthouse CI。`lighthouserc.json` でホーム・商品・記事・診断・お気に入り・ランキングの6ページを計測。**閾値はページ種別で異なる**(26章c29の実測に基づく個別設定・詳細は下記):

   | 対象 | Performance | Accessibility / Best Practices | SEO |
   |---|---|---|---|
   | `/favorites/`(client-renderedかつ意図的にnoindex) | 0.75以上 | 0.95以上 | 0.90以上 |
   | それ以外の全ページ | 0.90以上 | 0.95以上 | 0.95以上 |

   Performanceを当初計画の「全ページ0.95以上」から緩和した経緯は [docs/26-sonnet-implementation-spec-v3.md](docs/26-sonnet-implementation-spec-v3.md) §4「実装結果」を参照。

週次で `.github/workflows/link-check.yml`(26章c14)・`rewrite-suggest.yml`(26章c25)・`weekly-report.yml`(26章c26)も自動実行されます。詳細は下記「オーナーへの引き継ぎ事項」12・13を参照。
...
このサイトは**完全静的サイト**(`astro.config.mjs` に `output`/adapter指定なし=デフォルトの `output: 'static'`)としてビルドされ、`dist/` をそのまま配信します。ページ本体はSSR不要のため**`@astrojs/cloudflare` アダプターは使用していません**。ただし26章c11以降、クリック計測用に `functions/go/[[route]].ts`(Cloudflare Pages Functions)を追加しています。Pages Functionsはリポジトリ直下の `functions/` を自動検出する仕組みのため、これも**`wrangler.toml` は不要**です([docs/08-tech-architecture.md](docs/08-tech-architecture.md) §5)。
...
6. 独自ドメイン取得後は Pagesプロジェクトの **Custom domains** で追加し、`astro.config.mjs` の `site` を実ドメインに変更(現在は実デプロイ先 `https://lifestyle-ako.pages.dev`)
...
### クリック計測用 Pages Functions(26章c11)

`/go/{productId}/{mall}?pos={position}` にアクセスすると、`functions/go/[[route]].ts` が `dist/api/affiliate-map.json`(ビルド時生成)を参照して実際のアフィリエイトURLへ302リダイレクトします。存在しない商品・モールの組み合わせは404(本文に `/products/` への導線あり)。

クリックの記録には[Analytics Engine](https://developers.cloudflare.com/analytics/analytics-engine/)のバインディングを使いますが、**未設定でもリダイレクト自体は問題なく動作します**(バインディング未設定時は計測をスキップするだけ)。記録を有効にする場合:

1. ダッシュボード > **Workers & Pages > (対象プロジェクト) > Settings > Functions > Analytics Engine bindings** で新規追加
2. 変数名を `CLICKS` に設定(コード側でこの名前を参照しているため必須)、データセット名は任意
3. 保存後、次のデプロイから `env.CLICKS` が有効になりクリックごとに1レコード書き込まれる

ローカル動作確認は `astro dev` ではなく `wrangler pages dev`(要 `npm run build` を先に実行)で行うこと。`astro dev` は `functions/` を認識しません。

```sh
npm run build
npx wrangler pages dev dist --port 8788
curl -i "http://localhost:8788/go/{productId}/{mall}?pos=test"
```
...
4. **サンプル写真から実写真への差し替え** — `src/assets/products/` `src/assets/articles/` `src/assets/categories/` `src/assets/site/`(`instagram/` 配下6枚を含む)のプレースホルダーSVGを実写真(JPEG/PNG)に差し替え、各商品・記事・`site.json` の画像パスを更新。AI生成画像を使う場合は19章の運用ルール(`ai-` プレフィックス・alt明記)に従うこと。実写真は長辺1600px以下・1MB以下を目安にすること(超過はCIの`npm run check:images`が警告するが、ビルドは失敗しない・26章c28)
...
6. **Instagram/Threadsのフォロー導線URL確認** — `site.json` の `sns.instagramHome` / `sns.threads` 等は仮のプレースホルダーURL(`https://www.instagram.com/` 等)のままです。トップページのInstagramセクション・Follow CTAで実際に使われるため、公開前に実アカウントのURLへ差し替えてください。同じ値が `/about/` ページのPerson構造化データ(`sameAs`・26章c27)にもそのまま使われるため、ここを実URLへ差し替えるとE-E-A-T上の紐付けも合わせて正しくなります
...
13. **週次分析レポートの有効化(任意・26章c26)** — `.github/workflows/weekly-report.yml` は毎週月曜10:00 JSTにWeb Analyticsのページビュー・`/go/`クリック数を集計し `weekly-report`ラベル付きのIssueへ投稿します。有効化する場合、以下3つをGitHub Secretsに設定してください:
    - `CF_API_TOKEN` — Account Analytics:Read権限を持つCloudflareのAPIトークン
...
14. **管理画面 `/admin/`(任意)** — `/go/` クリック数を商品別・日別に確認できるページです。`functions/admin/_middleware.ts` によるBasic Auth(簡易パスワード)で保護されています。有効化する場合、Cloudflare Pages → 対象プロジェクト → **Settings > Variables and secrets** で以下をSecretとして追加してください:
    ...
    いずれか未設定の間は該当箇所が503エラーになるだけで、他の機能には影響しません(0章§0前提)。
