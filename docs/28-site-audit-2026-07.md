...
- [x] 390px幅のトップページで、お気に入りボタンのタップ判定領域(擬似要素を含む)が44×44px以上になる
- [x] ボタンのアイコン自体の見た目の大きさが変更前と同一(36px)
- [x] 拡張した領域が隣接するリンク(商品名リンク等)の上に重なってクリックを奪っていない
- [x] お気に入りの登録・解除が従来どおり動作する(クリックして状態が切り替わることを確認)
- [x] `npm run build` が0エラー
...
- [x] **課題3-a**: `eufy-solocam-s340-review`(264字)をヒアリング→二段階改稿(実装済み・2515字)
- [x] **課題9-a**: 同記事の末尾CTAを追加(課題3-aと同時・実装済み)
- [x] **課題3-b**: `nahe-shopper-m-first-look`(380字)をヒアリング→二段階改稿(実装済み・2525字)
- [x] **課題9-b**: 同記事の末尾CTAを追加(実装済み)
- [x] **課題3-c**: `nordic-paper-cord-dining-chair-review`(833字)をヒアリング→二段階改稿(実装済み・2501字)
- [x] **課題9-c**: 同記事の末尾CTAを追加(実装済み)
- [x] **課題3-d**: `fufu-kyoto-family-trip`(1523字)をヒアリング→二段階改稿(実装済み・2500字)
- [x] **課題9-d**: 同記事の末尾CTAを追加(実装済み)

**次のスプリント完了(2026-07-28)**: 記事4本(eufy/nahe/nordic/fufu)の改稿と末尾CTA追加をすべて実施済み。全記事が2500字以上・H2 4〜6・各H3 2〜4・文長/段落ルール・禁止語なしを機械チェック済み。
- [x] **課題12**: Aboutのレビューポリシー拡張(3-4の確認事項1の回答後・実装済み)

記事は**1本ずつ完結させてからcommit**する。4本まとめてヒアリングし、まとめて改稿しないこと(採点と差し戻しが混ざるため)。

### 中長期

- [x] **課題8**: トップページに診断への導線セクションを追加(実装済み・2026-07-28)
- [x] **課題11**: 診断結果から対応レビュー記事へのリンクを追加(実装済み・2026-07-28。`getArticlesForProduct`による自己整合的な逆引きを使用)
- [x] **課題13**: `FavoriteButton --sm` のタップ領域を44pxに拡張(実装済み・2026-07-29。`::after`疑似要素で見た目36pxのまま判定領域のみ拡張、実測確認済み)
- [x] 子育てカテゴリのコンテンツ拡充(2026-07-28: コンビTHE Sシリーズ3点セットの商品3点・記事1本を追加。ヘッダーナビに「子育て」が復帰)
- [ ] 写真カテゴリのコンテンツ拡充(未着手)

---

## 3-4. 判断が必要な確認事項(オーナーの意思決定・回答済み)

2026-07-28、オーナー(きのこ)より以下の回答を得た。

1. **レビューポリシーの記載内容(課題12)** → **承認・実装済み**
   「掲載商品は原則としてすべて自費購入。提供品の場合は記事内に明記」と明記してよい。現在の全6商品は自費購入という理解で正しい。
   `src/pages/about.astro` に反映済み(本文829字、1文60字以内・1段落4文以内を確認)。

2. **空カテゴリ「子育て」「写真」の扱い** → **(a) を採用・変更なし**
   課題2の実装(空状態の案内を出して残す)のまま運用する。子育てはヘッダーナビ上位5件に入る `order: 2` のため、記事・商品が1件以上できれば課題1のロジックにより自動的にナビへ復帰する。

3. **トップページのtitle文言(課題6)** → **文言変更・実装済み**
   `HIBISTACK | 実際に買って使った、正直な暮らしのレビュー` を採用(オーナーの選定案。検索語「実際に買って使った」を含む)。
   `src/pages/index.astro` に反映済み。

4. **記事4本の改稿の優先順位** → **字数の少ない順を採用**
   本書の提案どおり `eufy-solocam-s340-review` → `nahe-shopper-m-first-look` → `nordic-paper-cord-dining-chair-review` → `fufu-kyoto-family-trip` の順で、次のスプリントに進める。

5. **診断結果に表示する記事リンクの文言(課題11)** → **本書の提案「使ってみた感想を読む」を採用**
   2026-07-28、フェーズ1の一環として実装済み。
