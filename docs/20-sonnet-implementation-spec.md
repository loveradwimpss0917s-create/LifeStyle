...
## 12. 実装結果(c1〜c10 + ダークモード遷移バグ修正、完了)

c1〜c10はすべてコミット済み(`main`反映済み)。実装時に発生した設計との差異は以下の2点。

### 12.1 目次(Toc.astro)をsticky sidebarからdetails折りたたみへ簡略化(c8)

17章§5は「≥1024pxはsticky sidebar、<1024pxはdetails折りたたみ」としていたが、
ArticleLayoutが`.container-text`による単一カラム中央寄せ構成のため、sticky sidebarの
実現には本文エリアを2カラムグリッド化する構造変更が必要になる。実装時にリスクと
価値を比較し、全画面幅で`<details>`折りたたみに統一する簡略化を行った
(IntersectionObserverによる現在地ハイライトは仕様通り実装済み)。
sticky sidebar化は将来のバックログとして残す(残課題)。

### 12.2 View Transitionsの商品画像morph(2.7)を見送り(c10)

カード⇄商品ページ間で同一画像を`transition:name`により連続的に変形させる演出は、
カード表示(4:5クロップ)と商品詳細のGallery表示(元画像の縦横比のまま)でアスペクト比が
異なるため、morph時に不自然な歪みが生じるリスクがあると判断し実装を見送った。
実施する場合はGalleryのメイン画像も4:5に統一するか、別途morph専用の要素を用意する
設計変更が前提になる。

### 12.3 ダークモードがページ遷移で解除される不具合(c1〜c4完了後に発覚・別途修正済み)

c4実装時点では「`<html>`要素はView Transitions遷移をまたいで保持されるため
`data-theme`属性は自然に引き継がれる」と想定していたが誤りだった。実際のAstro
ClientRouterのswap処理は遷移先ページのSSR済みHTML(サーバーはクライアントの
テーマ設定を知り得ないため常に`data-theme`属性を持たない)を基準に`<html>`の属性を
再構築するため、遷移のたびにダークモードが解除される不具合が実機で発覚した。
`astro:before-swap`イベントで遷移先ドキュメントへ現在のテーマを明示的にコピーする
方式に修正済み(BaseLayout.astro)。同種の「View Transitions下での状態保持」を
今後実装する際は、`<html>`/`<head>`の属性は自動的には引き継がれない前提で設計すること。

### 12.4 残課題(Low・未着手)

- 目次のsticky sidebar化(12.1参照)
- View Transitionsの商品画像morph(12.2参照)
- 検索結果プレースホルダー(Pagefind読み込み待ちのスケルトン表示)は未着手
- lighthouserc.jsonへのダークテーマ計測追加は見送り(CIの実行時間増とdata-theme
  切り替えを伴う計測設定の複雑さに対し、優先度が低いと判断)
